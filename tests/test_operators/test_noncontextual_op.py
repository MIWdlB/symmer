...
def test_noncon_input():
    ...
    Hnc = NoncontextualOp.from_dictionary(
        {
            "ZIIIIYYX": (2.64309332429773 + 0j),
...
def test_from_hamiltonian_diag():
    ...
    assert np.all(np.sum(H_noncon_diag.X_block, axis=1) == 0), (
...
def test_solve_brute_force_discrete_no_ref():
    ...
    H_noncon.solve(strategy="brute_force", ref_state=None)
...
def test_solve_binary_relaxation_no_ref():
    ...
    H_noncon.solve(strategy="binary_relaxation", ref_state=None)
...
def test_solve_full_reference_state():
    ...
    H_noncon.solve(strategy="brute_force", ref_state=reference)
    assert np.isclose(H_noncon.energy, noncon_problem["E"])
...
def test_solve_brute_force_discrete_partial_ref():
    ...
    with pytest.warns():
        ...
        H_noncon.solve(strategy="brute_force", ref_state=partial_reference_state)
    assert np.isclose(H_noncon.energy, noncon_problem["E"])
