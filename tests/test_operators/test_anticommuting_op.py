...
def test_unitary_partitioning_no_s_index_seq_rot():
    ...
    assert np.isclose(gamma_l, np.linalg.norm(list(anti_commuting_real.values()))), (
        "normalization wrong"
    )
...
def test_unitary_partitioning_s_index_seq_rot():
    ...
    for s_ind in range(AcOp_real.n_terms):
        ...
        assert Ps.n_terms == 1, "can only rotate onto single Pauli operator"
        assert np.isclose(
            gamma_l, np.linalg.norm(list(anti_commuting_real.values()))
        ), "normalization wrong"
        assert len(rotations) == AcOp_real.n_terms - 1, (
