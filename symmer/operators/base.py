...
from .utils import (
    ...
    binary_array_to_int,
    check_adjmat_noncontextual,
    check_independent,
    check_jordan_independent,
    cref_binary,
    matmul_GF2,
    random_symplectic_matrix,
    string_to_symplectic,
    symplectic_cleanup,
    symplectic_to_openfermion,
...
class PauliwordOp:
    ...
    def __init__(
        ...
        if symp_matrix.dtype == int:
            ...
            assert set(np.unique(symp_matrix)).issubset({0, 1}), (
        ...
        assert self.symp_matrix.shape[-1] % 2 == 0, (
            "symplectic matrix must have even number of columns"
        )
        assert len(self.symp_matrix.shape) == 2, (
        ...
        assert self.n_terms == len(self.coeff_vec), (
    ...
    def _from_matrix_projector(
        ...
        binary_vec = (
            (np.arange(2**n_qubits).reshape([-1, 1]) & (1 << np.arange(n_qubits))[::-1])
            > 0
        ).astype(bool)
    ...
    def generator_reconstruction(
        ...
        if not override_independence_check:
            assert check_independent(generators), (
    ...
    def jordan_generator_reconstruction(self, generators: "PauliwordOp"):
        ...
        Returns:
            Tuple[np.array, np.array]: A tuple containing the reconstruction matrix and a mask indicating which terms were successfully reconstructed.
        """
        assert check_jordan_independent(generators), (
    ...
    def append(self, PwordOp: "PauliwordOp") -> "PauliwordOp":
        ...
        """
        assert self.n_qubits == PwordOp.n_qubits, (
    ...
    def _multiply_by_operator(
        ...
        Q_symp_matrix = PwordOp.symp_matrix.reshape(
            [PwordOp.n_terms, 1, 2 * PwordOp.n_qubits]
        )
        termwise_phaseless_prod = self.symp_matrix ^ Q_symp_matrix
        Y_count_in = self.Y_count + PwordOp.Y_count.reshape(-1, 1)
        Y_count_out = np.sum(
            termwise_phaseless_prod[:, :, : PwordOp.n_qubits]
            & termwise_phaseless_prod[:, :, PwordOp.n_qubits :],
            axis=2,
        )
        sign_change = (-1) ** (
            np.sum(self.X_block & Q_symp_matrix[:, :, PwordOp.n_qubits :], axis=2) % 2
        )
        phase_mod = sign_change * (1j) ** ((3 * Y_count_in + Y_count_out) % 4)
        return PauliwordOp(
            *symplectic_cleanup(
                termwise_phaseless_prod.reshape(-1, 2 * self.n_qubits),
                (phase_mod * np.outer(self.coeff_vec, PwordOp.coeff_vec).T).reshape(-1),
    ...
    def __getitem__(self, key: Union[slice, int]) -> "PauliwordOp":
        ...
        if isinstance(key, int):
            ...
            raise ValueError(
                f"Unrecognised input {type(key)}, must be an integer, slice, list or np.array"
            )
    ...
    def commutes_termwise(self, PwordOp: "PauliwordOp") -> np.array:
        ...
        **example
        ...
        return ~matmul_GF2(
            self.symp_matrix, np.hstack((PwordOp.Z_block, PwordOp.X_block)).T
        )
    ...
    def _rotate_by_single_Pword(
        self, Pword: "PauliwordOp", angle: float = None, threshold: float = 1e-18
    ) -> "PauliwordOp":
        ...
        if np.all(commute_vec):
            ...
            if abs(int_part - multiple) <= threshold:
                ...
                if abs(angle) > 1e6:
                    warnings.warn(
                        "Large angle can lead to precision errors: recommend using high-precision math library such as mpmath or redefine angle in range [-pi, pi]"
                    )
    ...
    def perform_rotations(
        ...
        if rotations == []:
            ...
            for pauli_rotation, angle in rotations:
                op_copy = op_copy._rotate_by_single_Pword(
                    pauli_rotation, angle
                ).cleanup()
            return op_copy
    ...
    def tensor(self, right_op: "PauliwordOp") -> "PauliwordOp":
        ...
        """
        identity_block_right = np.zeros(
            [right_op.n_terms, self.n_qubits], dtype=bool
        )  # .astype(int)
        identity_block_left = np.zeros(
            [self.n_terms, right_op.n_qubits], dtype=bool
        )  # .astype(int)
    ...
    def get_graph(
        self, edge_relation: Optional[str] = "C", label_nodes: Optional[bool] = False
    ) -> nx.graph:
    ...
    def generators(self) -> "PauliwordOp":
        ...
        assert generators.n_terms <= 2 * self.n_qubits, (
...
class QuantumState:
    ...
    def __mul__(
        ...
        assert self.n_qubits == mul_obj.n_qubits, (
    ...
    def from_array(
        ...
        **example
            ...
            >>  0.5773502692 |000> +
                0.8164965809 |101>
        """
        assert (len(statevector.shape) == 2) and (1 in statevector.shape), (
        ...
        if not np.isclose(np.linalg.norm(statevector), 1):
            warnings.warn("statevector is not normalized")
        ...
        if N < 64:
            state_matrix = ((non_zero[:, None] & (1 << np.arange(N))[::-1]) > 0).astype(
                int
            )
        else:
            state_matrix = (
...
def change_of_basis_XY_to_Z(P_op: PauliwordOp) -> PauliwordOp:
    ...
    if n_Sdag == 0:
        ...
        Z_block = (
            (np.arange(2**n_Sdag).reshape([-1, 1]) & (1 << np.arange(n_Sdag))[::-1]) > 0
        ).astype(bool)
