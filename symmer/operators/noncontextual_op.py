...
class NoncontextualOp(PauliwordOp):
    ...
    def random(
        ...
        """
        assert n_cliques > 1 or n_cliques == 0, (
            "number of cliques must be zero or set to 2 or more (cannot have one anticommuting term)"
        )
        n_clique_qubits = int(np.ceil((n_cliques - 1) / 2))
        assert n_clique_qubits <= n_qubits, (
            f"cannot have {n_cliques} anticommuting cliques on {n_qubits} qubits"
        )

        remaining_qubits = n_qubits - n_clique_qubits

        if n_commuting_terms:
            assert n_commuting_terms <= 2**remaining_qubits, (
        ...
        if remaining_qubits >= 1:
            if n_commuting_terms is None:
                ...
                indices = np.random.choice(
                    np.arange(0, 2**remaining_qubits),
        ...
        if n_cliques == 0:
            ...
            if n_commuting_terms > 0:
                ...
                assert (
                    n_commuting_terms * n_cliques + n_commuting_terms == H_nc.n_terms
        ...
        if apply_clifford:
            ...
            for _ in range(n_qubits * 5):
                ...
                P_rand.coeff_vec = [1]
                U_cliff_rotations.append(
                    (P_rand, (np.pi / 2) * np.random.choice([1, 3]))
                )
    ...
    def solve(self, strategy: str = "brute_force", ref_state: np.array = None) -> None:
    ...
    def noncon_state(self, UP_method="LCU") -> Tuple[QuantumState, np.array]:
        ...
        if UP_method == "LCU":
            ...
            Ps, rotations_LCU, gamma_l, AC_normed = (
                self.clique_operator.unitary_partitioning(s_index=0, up_method="LCU")
            )
        else:
            Ps, rotations_SEQ, gamma_l, AC_normed = (
                self.clique_operator.unitary_partitioning(
...
class NoncontextualSolver:
    ...
    def __init__(
        ...
        if fixed_ev_mask is not None:
            ...
            assert np.sum(fixed_ev_mask) == len(fixed_eigvals), (
