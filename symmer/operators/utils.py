...
def numba_dot_matmal_GF2(A, B):
    ...
    """
    return np.asarray(
        np.dot(np.asarray(A, dtype=np.float64), np.asarray(B, dtype=np.float64)) % 2,
        dtype=np.bool_,
    )
...
def string_to_symplectic(pauli_str, n_qubits):
    ...
    Returns:
        symp_vec (array): symplectic Pauliword array
    """
    assert len(pauli_str) == n_qubits, (
        "Number of qubits is incompatible with pauli string"
    )
    assert set(pauli_str).issubset({"I", "X", "Y", "Z"}), (
...
def check_jordan_independent(operators):
    ...
    XZY_block = np.hstack(
        (
            np.logical_xor(operators.X_block, Y_block),
            np.hstack((np.logical_xor(operators.Z_block, Y_block), Y_block)),
        )
    )
...
def perform_noncontextual_sweep(operator) -> "PauliwordOp":
    ...
    for index, term in enumerate(operator[1:]):
        ...
        adjmat_vector = np.append(
            term.commutes_termwise(operator[noncon_indices]), True
        )
        adjmat_padded = np.pad(adjmat, pad_width=((0, 1), (0, 1)), mode="constant")
        adjmat_padded[-1, :] = adjmat_vector
        adjmat_padded[:, -1] = adjmat_vector
